...
/////////////////////////////////////////////////////////////////////////////
// CResizableDockContext
// static members

CResizableDockContext::CResizableDockContext(CControlBar* pBar) : CDockContext(pBar)
{
//TRACE0("CResizableDockContext::CResizableDockContext\n");
}


CResizableDockContext::~CResizableDockContext()
{
//	TRACE0("CResizableDockContext::~CResizableDockContext\n");
}
...
/////////////////////////////////////////////////////////////////////////////
// CResizableDockContext operations

void CResizableDockContext::StartDrag(CPoint pt)
{
//TRACE0("----CResizableDockContext::StartDrag\n");
   	ASSERT_VALID(m_pBar);
   	ASSERT(m_pBar->IsKindOf(RUNTIME_CLASS(CResizableControlBar)));
   
   	CResizableControlBar *pSzBar = (CResizableControlBar *) m_pBar;
	...
	// check to see we're not hanging from a CResizableFloatWnd. Disallow dragging if we are...
	if (m_pBar->IsFloating())
	{
		CFrameWnd * pFrameWnd = m_pBar->GetParentFrame();
		ASSERT(pFrameWnd != NULL);
		ASSERT(pFrameWnd->IsKindOf(RUNTIME_CLASS(CFrameWnd)));
		if (pFrameWnd->IsKindOf(RUNTIME_CLASS(CResizableFloatWnd)))
			return;				// do nothing if floating inside a CResizableFloatWnd
	}

   // dragging has started message (only if window will actually dock !)
    if ((m_dwDockStyle & CBRS_ALIGN_ANY) != 0)
    	AfxGetMainWnd()->SendMessage(WM_SETMESSAGESTRING, IDS_BAR_STARTDOCKING);
...
void CResizableDockContext::Move(CPoint pt)
{
//TRACE0("CResizableDockContext::Move\n");
...
void CResizableDockContext::OnKey(int nChar, BOOL bDown)
{
//TRACE0("CResizableDockContext::OnKey\n");
	if (nChar == VK_CONTROL)
    	UpdateState(&m_bForceFrame, bDown);
}


void CResizableDockContext::EndDrag()
{
//TRACE0("CResizableDockContext::EndDrag\n");
    CancelDrag();
   ...
   m_dwOverDockStyle = m_bForceFrame ? 0 : CanDock();
	if (m_dwOverDockStyle != 0)
    {
        ...
        // check the original dockbar - if a valid CResizableDockBar...work out the row number.
		CDockBar * pOrigDockBar = m_pBar->m_pDockBar;
		int nOrigCheckSum = -1;
		if (pOrigDockBar != NULL && pOrigDockBar->IsKindOf(RUNTIME_CLASS(CResizableDockBar)))
			nOrigCheckSum = ((CResizableDockBar *)pOrigDockBar)->CheckSumBars();

		// Now we're going to actually dock the window.
		// Update the appropriate size in the control bar.
		if (HORZF(m_dwOverDockStyle)) 
        	((CResizableControlBar *) m_pBar)->m_HorzDockSize = m_rectDragDock.Size();
		else
			((CResizableControlBar *) m_pBar)->m_VertDockSize = m_rectDragDock.Size();
		...
		if (pDockBar->IsKindOf(RUNTIME_CLASS(CResizableDockBar)))
		{
			if (pOrigDockBar != pDockBar || ((CResizableDockBar*)pDockBar)->CheckSumBars() != nOrigCheckSum)
			{
				((CResizableDockBar *)pDockBar)->AdjustForNewBar(m_pBar);
			}
			...
			((CResizableDockBar *)pDockBar)->m_CountBars = 0;			
		}
...
void CResizableDockContext::CancelDrag()
{
//TRACE0("CResizableDockContext::CancelDrag\n");
...
void CResizableDockContext::DrawFocusRect(BOOL bRemoveRect)
{
//TRACE0("CResizableDockContext::DrawFocusRect\n");
	ASSERT(m_pDC != NULL);
...
void CResizableDockContext::UpdateState(BOOL* pFlag, BOOL bNewValue)
{
//TRACE0("CResizableDockContext::UpdateState\n");
    if (*pFlag != bNewValue)
    {
    	*pFlag = bNewValue;
...
DWORD CResizableDockContext::CanDock()
{
//TRACE0("CResizableDockContext::CanDock\n");
	BOOL bStyleHorz;
...
CDockBar* CResizableDockContext::GetDockBar()
{
//TRACE0("CResizableDockContext::GetDockBar\n");
	DWORD dw = 0;
    CDockBar* pBar;
    if (HORZF(m_dwOverDockStyle))
    {
...
BOOL CResizableDockContext::Track()
{
//TRACE0("CResizableDockContext::Track\n");
...
void CResizableDockContext::ToggleDocking()
{
//TRACE0("CResizableDockContext::ToggleDocking\n");
   	ASSERT_VALID(m_pBar);
   	ASSERT(m_pBar->IsKindOf(RUNTIME_CLASS(CResizableControlBar)));

	// check to see we're not hanging from a CResizableFloatWnd. Disallow toggle docking if we are
	if (m_pBar->IsFloating())
	{
		CFrameWnd * pFrameWnd = m_pBar->GetParentFrame();
		ASSERT(pFrameWnd != NULL);
